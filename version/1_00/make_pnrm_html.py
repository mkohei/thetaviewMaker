...
# CONTENT_HTML % (title, date, list_item<li>)
CONTENT_HTML = """
<!DOCTYPE html>
<html>
<head>
...
<li>
    <dl>
        <dt><a href="view/%s.html"><img src="pics/%s.jpg" alt="Photo" width="200" height="100"></a></dt>
        <dd><strong><a href="view/%s.html">%s</a></strong><br> %s
        </dd>
    </dl>
</li>
"""
...
def makehtml(imgs, path, title=None):
    ...
    open('%s/index.html' % path, 'w').write(CONTENT_HTML % (title if title is not None else 'untitled', str(datetime.date.today()), list_items))
    return
...
if __name__ == '__main__':
    ...
    if SETTING is not None:
        ...
        for params in SETTING:
            ...
            if 'filename' not in params:
                print("[Name Key Existence Error] `name` key is undefined.")
                continue
            ...
            if 'description' not in params:
                print("[Warning] `description key is")
                description = None
            else:
    ...
    prog = re.compile('.*\\.jpg') # Preparation of (:*1)
    for file in os.listdir('%s/pics/' % PATH):
        # do processing only JPEG file (you cat change only JPEG file -> image files) (:*1)
        if bool(prog.match(file)) is False:
            continue
